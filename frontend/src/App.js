  ...
  renderItems = () => {
    ...
    return newItems.map((item) => (
      <li key={item.id} className="list-group-item d-flex justify-content-between align-items-center">
        ...
        <span><button className="btn btn-secondary mr-2" onClick={() => this.editItem(item)}>Edit</button></span>
        <span><button className="btn btn-danger" onClick={() => this.handleDelete(item)}>Delete</button></span>
      </li>
    ));
  };
  ...
  render() {
    return (
      <main className="container">
        ...
        {
          this.state.modal && <CustomModal onSave={this.handleSubmit} toggle={this.toggle} activeItem={this.state.activeItem} />
        }
      </main>
    )
  }
